...
import { IEditPatient } from '../../../../services/PatientService';


class PatientDashboardEditProperties extends BasePageProperties {
    match?: {
        params: {
            pId: string;
        }
...
interface PatientDashboardEditState {
    forename: string;
    surname: string;
    bornAt: string;
    gender: "male" | "female";
...
export default class PatientDashboardEdit extends BasePage<PatientDashboardEditProperties> {
    state: PatientDashboardEditState;

    constructor(props: PatientDashboardEditProperties) {
        super(props);
    ...
    private getPatient() {
        PatientService.getPatientById(this.getPatientId())
            .then(res => {
                if (res === null) {
                    this.setState({
                        errorMessage: "Nije moguća izmena pacijenta, jer je došlo do greške prilikom učitavanja..."
                    });
                    return;
                }
                ...
                this.setState({
                    ...
                    surname: res.surname,
                    bornAt: res.bornAt.toString().substring(0, 10),
                    gender: res.gender,
                    email: res.email,
                    personalIdentityNumber: res.personalIdentityNumber,
                    address: res.address,
